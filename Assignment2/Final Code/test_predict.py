...
for fn in models_fn:
    ...
    mdl_name = type(clf).__name__

    np.save(f'{TEST_PREDICTION_FOLDER}{mdl_name}_proba_order.npy', clf.classes_)
    print(f"Class Prediction")
    y_pred = clf.predict(X_test)
    save_npy_gzip(f'{TEST_PREDICTION_FOLDER}{mdl_name}_pred.npy', y_pred)
